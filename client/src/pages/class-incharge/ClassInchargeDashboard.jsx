...
export default function ClassInchargeDashboard() {
  ...
  const fetchClearances = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get('/clearances/ci/pending');
  ...
  const stats = useMemo(() => {
    const pending = clearances.filter((c) => c.status === 'ci_review' || c.status === 'pending').length;
    const approved = clearances.filter((c) => c.status === 'approved' || c.status === 'hod_review' || c.status === 'completed').length;
  ...
  const handleReview = async () => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/clearances/ci/${selectedItem._id}/review`, {
        status: modalAction,
        remarks: remarks.trim(),
      });
      toast.success(
        modalAction === 'approved'
          ? 'Clearance advanced to HOD review'
          : 'Clearance rejected'
      );
      closeModal();
      fetchClearances();
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      render: (_, row) => {
        const isPending = row.status === 'ci_review' || row.status === 'pending';
        if (!isPending) {
  ...
  const statCards = [
    { label: 'Pending CI Reviews', value: stats.pending, color: 'text-status-pending' },
  ...
  return (
    <DashboardLayout title="Class Incharge Dashboard">
      {/* Stats */}
      ...
      {/* Table */}
      <div className="mb-2">
        <h2 className="text-base font-semibold text-ink-primary mb-3">Pending CI Reviews</h2>
      </div>
      <Table
        columns={columns}
        data={clearances}
        loading={loading}
        emptyMessage="No pending CI reviews"
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label htmlFor="ci-review-remarks" className="label-base">
              Remarks
            </label>
            <textarea
